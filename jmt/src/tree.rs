...
// #[cfg(not(any(test, features = "mocks")))]
// impl<'a, R, H> Default for JellyfishMerkleTree<'a, R, H>
// where
//     R: TreeReader + VersionedDatabase + Default,
//     H: SimpleHasher,
// {
//     fn default() -> Self {
//         let db_wrapped = Arc::new(R::default());
//         let raw_ptr = Arc::as_ptr(&db_wrapped);
//         let mut db = unsafe { &*raw_ptr.clone() };
//         JellyfishMerkleTree::new(&mut db)
//     }
// }
    ...
    H: SimpleHasher,
{
    ...
    pub fn new_migration(reader: &'a mut R, leaf_count_migration: bool) -> Self {
        Self {
            reader,
            leaf_count_migration,
            _phantom_hasher: Default::default(),
